...
const querySourceInfo = async (data, db, strMemberId, date, time) => {
  ...
  // 将溯源主信息加入到刚注册用户中
  if (objSourceInfo) {
    console.log("querySourceInfo", objSourceInfo.data.MEMBER_nickName);
    try {
      await db
        .collection("MEMBER")
        .doc(strMemberId)
        .update({
          data: {
            share_sourceNickName: db.command.set(
              objSourceInfo.data.MEMBER_nickName
            ),
          },
        });
    } catch (e) {
...
const createMember = async (data, db, strMemberId, date, time) => {
  ...
  const objMember = {
    ...
    // 个人信息
    MEMBER_openid: strMemberId.substr(4),
    MEMBER_nickName: data.nickName, // 昵称*
    MEMBER_avatarUrl: data.avatarUrl, // 头像*
    MEMBER_gender: data.gender, // 性别*
    MEMBER_country: data.country, // 国家*
    MEMBER_province: data.province, // 省份*
    MEMBER_city: data.city, // 城市*
    MEMBER_language: data.language, // 语言*
    MEMBER_cellphone: data.cellphone, // 手机号
    // 应用信息
    data_level: 1, // 等级
    data_exp: 0, // 经验
  ...
  // 创建新的玩家信息
  try {
    await db.collection("MEMBER").add({ data: objMember });
    objResult = {
      code: 200,
      data: { data: objMember },
    };
